...
export async function GET(request: NextRequest) {
  try {
    ...
    const pageSizeParam =
      Number(request.nextUrl.searchParams.get("pageSize") || request.nextUrl.searchParams.get("limit") || "10");
    const page = Math.max(Number.isFinite(pageParam) ? Math.floor(pageParam) : 1, 1);
    const pageSize = Math.min(
      Math.max(Number.isFinite(pageSizeParam) ? Math.floor(pageSizeParam) : 10, 1),
      100
    );
